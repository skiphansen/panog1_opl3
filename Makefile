...
FW_BIN := $(TOPDIR)/fw/hello_world/midi_box.bin
MCS_FILE := $(TOPDIR)/xilinx/pano_midibox_fm.mcs
CREATE_MIF := $(TOPDIR)/tools/create_mif.rb
TOOLCHAIN_PREFIX = riscv32-unknown-elf-
CLEAN_FILES := *.elf *.bin *.mif *.map *.mem *.dump *.lst *.d *.o
...
.PHONY: prog_msc prog_all prog_fpga reload clean real_clean
